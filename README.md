...
A generic, lightweight Rust library for creating and managing blockchain blocks with persistent storage. This library provides core block structures, cryptographic primitives, and SledDB-backed persistence while remaining agnostic to the actual data you store in blocks. To create or open a blockchain, the library requires the path to a Openssl RSA Private Key in PEM format. On Linux, I use the openssl command to generate a 4096 bit application key. The library will prompt the user for the password on the command line. The RSA Private Key is used to encrypt/decrypt AES Keys. Unique AES GCM 256 bit keys are generated for each block transaction. Once the RSA Private Key is loaded at application start, it can be removed from the file system and returned to its secure offline storage.

The anotomy of a block is:
Block Header - static size represented by a public constant BLOCK_HEADER_SIZE
    block_uid - auto generated by UUIDV4
    version - constant BLOCK_VERSION
    parent_hash - SHA512 hash that is auto populated based on the state of the blockchain
    timestamp - auto generated current time value
    nonce - random number for entropy and future expansion
Block Hash - SHA512 hash of the Block Header
Block Data - AES GCM encrypted data that is tamper resistant based on AES 256 bit keys
